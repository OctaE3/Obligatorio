...
const DeleteVehicle = ({ navigation }) => {
  ...
  const vehiculosIncluidos = () => {
    ...
    //Comprueba que el vehiculo no este incluido en usuarios o tratamientos
    db.transaction((tx) => {
  ...
  const deletevehicle = () => {
    ...
    if (!marca.trim()) {
      Alert.alert("Se requiere mostrar el vehiculo a eliminar");
      return;
    }

    console.log("Incluidos", incluido, incluidoR)
    //Si el vehiculo no esta inculido en usuarios o tratamientos elimina mediante una consulta sql 
